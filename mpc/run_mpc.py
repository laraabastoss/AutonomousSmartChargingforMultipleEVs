...
def run(method=V2GProfitMaxLoadsOracle, seed=5, verbose=False):
    ...
    for t in range(env.simulation_length):
        ...
        if verbose: print(f'Current timestep: {t+1} | Action: {actions}', end='\r')
...
if __name__ == "__main__":
    ...
    metrics = ['average_user_satisfaction', 'total_profits', 'tracking_error', 
               'power_tracker_violation', 'total_energy_charged', 'total_energy_discharged', 
               'battery_degradation', 'total_transformer_overload']
    ...
    for metric, values in results.items():
        print(f'Metric: {metric} | Mean: {np.round(np.mean(values), 3)} | STD: {np.round(np.std(values), 3)}')
