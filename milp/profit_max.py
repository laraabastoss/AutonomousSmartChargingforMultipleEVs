...
class V2GProfitMaxOracleGB:
    ...
    def __init__(
        ...
        energy = self.m.addVars(
            self.number_of_ports_per_cs,
            self.n_cs,
            self.sim_length,
        ...
        act_current_ev_dis = self.m.addVars(
            self.number_of_ports_per_cs,
            self.n_cs,
            self.sim_length,
            vtype=GRB.CONTINUOUS,
        ...
        act_current_ev_ch = self.m.addVars(
            self.number_of_ports_per_cs,
            self.n_cs,
            self.sim_length,
            vtype=GRB.CONTINUOUS,
        ...
        current_cs_ch = self.m.addVars(
            self.n_cs, self.sim_length, vtype=GRB.CONTINUOUS, name="current_cs_ch"
        )

        current_cs_dis = self.m.addVars(
            self.n_cs, self.sim_length, vtype=GRB.CONTINUOUS, name="current_cs_dis"
        )

        omega_ch = self.m.addVars(
            self.number_of_ports_per_cs,
            self.n_cs,
            self.sim_length,
            vtype=GRB.BINARY,
        ...
        current_tr_ch = self.m.addVars(
            self.n_transformers,
            self.sim_length,
            vtype=GRB.CONTINUOUS,
            name="current_tr_ch",
        )
        current_tr_dis = self.m.addVars(
            self.n_transformers,
            self.sim_length,
            vtype=GRB.CONTINUOUS,
            name="current_tr_dis",
        ...
        power_cs_ch = self.m.addVars(
            self.n_cs, self.sim_length, vtype=GRB.CONTINUOUS, name="power_cs_ch"
        )

        power_cs_dis = self.m.addVars(
        ...
        is_exceeding_limit = self.m.addVars(
            self.sim_length, vtype=GRB.BINARY, name="is_exceeding_limit"
        )

        user_satisfaction = self.m.addVars(
            self.number_of_ports_per_cs,
            self.n_cs,
            self.sim_length,
            vtype=GRB.CONTINUOUS,
        ...
        costs = self.m.addVar(vtype=GRB.CONTINUOUS, name="total_soc")
        # Constrains
        ...
        for t in range(self.sim_length):
            ...
            self.m.addConstr(
                total_power[t]
                == gp.quicksum(
                    power_tr_ch[i, t] - power_tr_dis[i, t]
                    for i in range(self.n_transformers)
                ),
        ...
        costs = gp.quicksum(
            ...
            * voltages[i]
            * cs_ch_efficiency[i, t]
            * dt
            * charge_prices[i, t]
            + act_current_ev_dis[p, i, t]
            * voltages[i]
        ...
        self.m.addConstrs(
            power_cs_ch[i, t] == (current_cs_ch[i, t] * voltages[i])
            for i in range(self.n_cs)
            for t in range(self.sim_length)
        )
        self.m.addConstrs(
            power_cs_dis[i, t] == (current_cs_dis[i, t] * voltages[i])
            for i in range(self.n_cs)
            for t in range(self.sim_length)
        )
        ...
        self.m.addConstrs(
            (
                current_tr_ch[i, t] - current_tr_dis[i, t] <= tra_max_amps[i, t]
                for i in range(self.n_transformers)
                for t in range(self.sim_length)
            ),
        ...
        self.m.addConstrs(
            (
                current_tr_ch[i, t] - current_tr_dis[i, t] >= tra_min_amps[i, t]
                for i in range(self.n_transformers)
                for t in range(self.sim_length)
            ),
        ...
        self.m.addConstrs(
            (
                current_cs_ch[i, t] == act_current_ev_ch.sum("*", i, t)
                for i in range(self.n_cs)
                for t in range(self.sim_length)
            ),
        ...
        self.m.addConstrs(
            (
                current_cs_dis[i, t] == act_current_ev_dis.sum("*", i, t)
                for i in range(self.n_cs)
                for t in range(self.sim_length)
            ),
        ...
        self.m.addConstrs(
            (
                -current_cs_dis[i, t] + current_cs_ch[i, t]
                >= port_max_discharge_current[i]
                for i in range(self.n_cs)
                for t in range(self.sim_length)
            ),
        ...
        self.m.addConstrs(
            (
                act_current_ev_ch[p, i, t] == current_ev_ch[p, i, t] * omega_ch[p, i, t]
                for p in range(self.number_of_ports_per_cs)
                for i in range(self.n_cs)
                for t in range(self.sim_length)
        ...
        self.m.addConstrs(
            (
                act_current_ev_dis[p, i, t]
                == current_ev_dis[p, i, t] * omega_dis[p, i, t]
                for p in range(self.number_of_ports_per_cs)
                for i in range(self.n_cs)
                for t in range(self.sim_length)
                #    if u[p, i, t] == 1 and ev_arrival[p, i, t] == 0
        ...
        self.m.addConstrs(
            (
                current_ev_ch[p, i, t]
                >= port_min_charge_current[i]  # * omega_ch[p, i, t]
                for p in range(self.number_of_ports_per_cs)
                for i in range(self.n_cs)
                for t in range(self.sim_length)
                #    if u[p, i, t] == 1 and ev_arrival[p, i, t] == 0
        ...
        self.m.addConstrs(
            (
                current_ev_dis[p, i, t]
                >= -port_min_discharge_current[i]  # * omega_dis[p, i, t]
                for p in range(self.number_of_ports_per_cs)
                for i in range(self.n_cs)
                for t in range(self.sim_length)
                #    if u[p, i, t] == 1 and ev_arrival[p, i, t] == 0
        ...
        self.m.addConstrs(
            (
                current_ev_ch[p, i, t]
                <= min(
                    ev_max_ch_power[p, i, t] / voltages[i], port_max_charge_current[i]
                )
                for p in range(self.number_of_ports_per_cs)
                for i in range(self.n_cs)
                for t in range(self.sim_length)
        ...
        self.m.addConstrs(
            (
                current_ev_dis[p, i, t]
                <= min(
                    -ev_max_dis_power[p, i, t] / voltages[i],
                    -port_max_discharge_current[i],
                )
                for p in range(self.number_of_ports_per_cs)
                for i in range(self.n_cs)
        ...
        self.m.addConstrs(
            (
                omega_dis[p, i, t] * omega_ch[p, i, t] == 0
                for p in range(self.number_of_ports_per_cs)
                for i in range(self.n_cs)
                for t in range(self.sim_length)
            ),
            ...
            for i in range(self.n_cs):
                for p in range(self.number_of_ports_per_cs):
                    if t_dep[p, i, t] == 1:
                        ...
                        self.m.addConstr(
                            user_satisfaction[p, i, t]
                            == (
                                ev_des_energy[p, i, t] * ev_max_energy[p, i, t - 1]
                                - energy[p, i, t]
                            )
                            ** 2,
    ...
    def get_actions(self):
        ...
        self.actions = np.zeros(
            [self.number_of_ports_per_cs, self.n_cs, self.sim_length]
        )
